...
- **Server components by default.** A file is a client component only when it needs state, effects or browser APIs. Client islands, and only these until a spec says otherwise: the comment composer, the classification card, votes, the Thinking Fingerprint, the opinion maps, the article editor (TipTap, `src/components/editor/`, backlog A-10). Keep each island small and pass it data from a server component.
  ...
  Reshaped the same day by the council's review of the maps (`council/log/2026-09-21-opinion-maps-and-the-declaration.md`, "Dan's decisions"). The Declare overlay runs Strongest Objection, the maps, Core Claim and Scope Boundary, then the engine's reading folded behind one `<details>` whose summary row carries the tier and the locked sentence ("disclosed alongside the article, never used to gate publication"), so it shows with no click; do not move that sentence back into the folded body. The detected core claim renders only when `ai_analysis.alignment` is `partial` or `divergent` (`components/opinion-map/data.ts` parses it; a row with no alignment word falls back to "show it if it differs"). Every map draws its question (the island now draws it on Declare, where no Reflect prompt already carries it) and a caption under its author marker, worded by the map's optional `author_position_source` (`'author'`, or `'engine'` when absent, which every live article is): `data.ts` reads it, nothing in this app writes it. The reader's marker is an ink diamond with a "You" tag, drawn after the author's brass circle in the same svg; SVG has no z-index, so document order is the z-order, and the two must stay in that order (the two `<g>` elements carry `data-marker="author"` and `data-marker="reader"`, so a DOM check can tell them apart). Marker dots take the muted `POLE_MARKER_COLORS` / `AXIS_MARKER_COLORS` (the bright landscape hues stay in the washes and the blend): a graphic needs 3:1 against its card and the gold dot measured 2.12:1. The Declare placement prints the same privacy sentence Reflect does; it is held once in `strings.ts` as `PLACEMENT_PRIVACY`, true because `opinion_map_positions` has the single `opinion_map_self_read` SELECT policy.
