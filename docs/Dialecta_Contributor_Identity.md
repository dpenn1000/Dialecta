...
Each archetype names a distinct cognitive move that the classification engine can detect from accumulated comment history. Together they cover the space without redundancy:
...
- **Pillar score computation** — the exact function from Stage A classification data to graduations and tier mix per pillar. Currently illustrative in the prototype.
