...
| **Its containment expires, and nothing in the thing that ends it would notice** | **Measured 2026-09-20:** `comments.status` defaults to `'pending_review'`, the only read policy is `USING (status = 'published')`, and all 3 comments are `pending_review`. Nothing is published, so a spoofed comment currently reaches nobody. **That is a column default holding, not a control anyone wrote.** The docblock puts promotion in Wait Window and Stage 2.5 in Phase 2. The day that pipeline ships, this stops being bounded, and the pipeline itself would not flag it, because from its side it is promoting comments exactly as intended. Fix the endpoint before the promotion path, not after |
