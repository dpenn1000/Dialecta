...
| **What the repo actually decided** | ADR-002 and `council/log/2026-09-19-p0-d2-login-methods.md` name **magic link plus Google only**. Google is in the pass-through group. If any provider outside that group is in scope, it was decided somewhere this repository does not record, and that is its own finding: the option set would be smaller than anything written down, and the difference between Google and Facebook or X is a different control model rather than a weighting |
