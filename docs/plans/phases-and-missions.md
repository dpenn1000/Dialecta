...
| **A live hole, separate from everything below, and it must not be merged with it** | The deployed `api/comment.js` establishes no caller identity at all. Its docblock asserts that `member_id` and `member_name` come from the profile rather than the request "so a caller cannot spoof identity", which is true of the name and false of the choice of whose name: the only check is that `member_uuid` is a non-empty string, and the caller supplies it. `security` grepped all 563 lines for authorization, x-ghost, ghostAdmin, signature, session, jwt and hmac; four hits, none a check, one of them a typeface. **Measured live:** `profiles_select` is `SELECT` to `public` `USING (true)`, `anon` holds column SELECT on `ghost_member_id`, `anon` holds INSERT on `comments`. No signup, no session, no account. This runs on `ghost_member_id` against the deployed API and has nothing to do with Supabase Auth, so it is live today while the social-login problem is prospective |
| **What the containment does and does not cover** | **Measured 2026-09-20.** `comments.status` defaults to `'pending_review'` and the only read policy is `USING (status = 'published')`, so the forged comment's **text** reaches no reader. That is a column default holding, not a control anyone wrote. **Two effects escape it.** `comment.js` runs on `SUPABASE_SERVICE_KEY`, which bypasses RLS entirely, and in the same flow it inserts `axis_events` and fires notifications by `member_id`, neither gated on publish status. So a forged comment permanently alters the named victim's fingerprint, and `axis_events` is append-only with a service-only SELECT policy, so the entry cannot be cleanly removed. Real people receive real notifications from a comment nobody wrote. `builder` found this and sharpened the blast radius past where `security` had scoped it. **Correcting my own earlier statement that a spoofed comment reaches nobody: the text reaches nobody, the fingerprint damage and the notification both land.** |
| **And the containment expires** | The docblock puts promotion in Wait Window and Stage 2.5 in Phase 2. The day that pipeline ships, even the text stops being contained, and the pipeline itself would not flag it, because from its side it is promoting comments exactly as intended. Fix the endpoint before the promotion path, not after |
| **Measured on the live project 2026-09-20, before building anything** | `auth.users` is **0**. `auth.identities` is **0**. Supabase Auth has never been used. The 14 profiles are keyed by `ghost_member_id text not null`, and **`profiles.user_id` does not exist on the live table at all**, so ADR-002's consequence line describing it as already keyed that way is true of the repo migration and not of the database. Two things follow. The takeover path is prospective rather than live, because there is no identity to take over yet. And the claim token can be built before the first link ever happens, so there is no migration of already-linked accounts to design around. This is the cheapest this control will ever be |
| **`config.Mailer.Autoconfirm` is a master switch, and it is not in the provider decision** | It sits in the same condition as the verification check in `DetermineAccountLinking`, so turning it on bypasses the gate for **every** provider at once, Google included. Read it off the live project's config rather than assuming, and never turn it on for an unrelated reason such as smoothing a test flow |
...
| **What the gate actually does** | Read from source by `security` on 2026-09-20, `DetermineAccountLinking` in `internal/models/linking.go`, supabase/auth master at `2e9ce6c8`. **The gate is real**: an email joins the match pool only when `email.Verified \|\| config.Mailer.Autoconfirm`, and when none passes the decision is always CreateAccount, never LinkAccount. **The gate is only as honest as the provider adapter feeding it.** Google, GitHub, GitLab, Discord, Azure, Keycloak and generic OIDC pass the provider's real claim through. Facebook on both paths, X, legacy Twitter, Apple's native ID token path and about eight smaller providers **hardcode `Verified` true** regardless of what the provider said. Carry the caveat as `security` wrote it: hardcoded true is not the same as false. Supabase is assuming those platforms only return confirmed emails, X's adapter has a comment asserting it, Facebook's has none, and whether it holds lives outside Supabase's source and is unestablished |
| **What the repo decided, and what Dan actually asked for** | ADR-002 and `council/log/2026-09-19-p0-d2-login-methods.md` name **magic link plus Google only**. Dan's actual ask, given to the `security` session on 2026-09-20 and verbatim: *"I would like the user to be able to login using familiar credentials (Google, facebook, X, etc..) Please investigate that from your perspective."* The "etc." is his. **Sourced to a session, not to this repository**, which is the finding rather than the provider list: a decision that exists only in a transcript is a decision the Council cannot see, argue or inherit. Two of the three named providers are in the hardcoding group |
| **The key choice, not the provider gap** | NIST SP 800-63-4, Federation volume, says the identifier to store is **`sub`, not the email address**. Dialecta joins on email twice: once in Supabase's linking and again in P0-6. A current normative source names the alternative outright, which reframes the claim token as fixing a key choice rather than patching a provider's behaviour |
| **The attack has a name and a paper** | Sudhodanan and Paverd, USENIX Security 2022: account pre-hijacking, at least 35 of 75 services vulnerable, and two of their five variants are exactly this shape. Their remedy is **proof of current control before merging**, which is the claim token arrived at independently by researchers who were not looking at Dialecta. Worth saying when the debate runs |
| **Per provider, documented rather than measured** | Labelled as `security` labelled it. **Meta** never calls the returned email verified anywhere in its own field docs, and it can be absent entirely. **X** returns `confirmed_email` with no stated verification definition, and has been pay-per-use since February 2026 with no free tier, making it the only provider on Dan's list carrying a recurring bill, which is `treasurer`'s input rather than security's. **Apple** Guideline 4.8 does not reach a website, so no obligation lurks there. **GitHub** is cleanest of all of them: an explicit verified boolean per address, no review, usable immediately. **The sign-in test is withdrawn**, not deferred: `security` settled `identity_data` from source and the field is not usable, so there is nothing to measure. The second control reads `auth.users.email_confirmed_at` instead |
| **The control, ranked first of three** | Issue each of the fourteen a **one-time claim token out of band** and require it to take over a legacy profile. A token is a secret Dan issues; an email address is a fact an attacker asserts. It is the only control still correct if a provider's own verification is compromised, and it is one table and one check. Second, as defence in depth: gate the claim on `auth.users.email_confirmed_at`. **Not `identity_data`, and `security` withdrew its own first version of this control on 2026-09-20 after reading the source.** `identity_data` is not the provider's claims: `createAccountFromExternalIdentity` fills it from a normalised struct, and Facebook, X, Twitter, GitHub, GitLab and Discord all set the honest `Emails[].Verified` while none of them touches `Metadata.EmailVerified`, which is never dropped when empty and so stores as **`false`**. Google's sync for it lives in a function its own comment calls legacy and says never runs. A control reading `identity_data->>'email_verified'` gets the wrong answer for most providers including Google, and gets it as a confident false rather than a null, so it would refuse every legitimate claim, and inverting the check would accept everything. `email_confirmed_at` is written by `User.Confirm`, whose OAuth call site is gated on the honest field. **Carry one Supabase bug into the build rather than discover it there:** `Identity.IsEmailVerified()` reads the dishonest key, and `UpdateUserEmailFromIdentities` calls it to decide whether to null `email_confirmed_at`, so a Facebook-only or X-only contributor who loses another identity can be silently unconfirmed. Nothing may treat `email_confirmed_at` as monotonic. Third, holding a second provider until the fourteen have claimed, is a hedge rather than a control and does nothing for whoever never claims, who are exactly the people nobody is watching |
