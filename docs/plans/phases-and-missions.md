...
| **Entry** | P0-D2 decided, Supabase schema settled, three blockers closed |
...
| **The one legal question** | Whether the six pillars and the archetype are sensitive data under the 2026 CTDPA amendment, which dropped the volume threshold. Decides whether Dialecta is in scope at 14 users. No source answers it |
| **Done** | A contributor signs in with magic link and with Google, an existing Ghost member is matched by email on first sign-in, and `reviewer` confirms no path writes a row without `auth.uid()` |
