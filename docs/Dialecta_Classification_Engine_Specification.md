...
| ⚡ **Stance** | Any level, tribal framing dominant | Tribal, rhetorical, or identity-signaling markers overshadow whatever claim may exist. A position planted rather than a conversation joined. |
| 🚫 **Breach** | N/A — personal attack | Content targets a person, not an idea. The Pact has been broken. |
...
⚡ Stance — Tribal framing, rhetoric, or identity signaling dominates. A position planted rather than a conversation joined.
🚫 Breach — Personal attack on a person, not an idea. The Pact has been broken.
...
**Static vs. Heat**
Both are emotionally charged. The discriminator is the primary function: if the comment is mainly *expressing feeling*, it is Heat. If it is mainly *signaling group membership*, it is Static. A comment can have elements of both — assign to the dominant function.
...
- Feeding the contributor identity layer (axis scores and archetype assignment), which aggregates Stage A data across each contributor's full comment history. See the Contributor Identity Specification.
...
- **Static/Heat test set** — this boundary warrants a dedicated library of labeled examples to improve consistency.
