...
| 2.6 | **`apps/web` is missing `noUncheckedIndexedAccess` and `exactOptionalPropertyTypes`** | `architect` measured the cost with a probe tsconfig: zero errors across 28 files | Two lines. Only gets more expensive as the port adds files |
