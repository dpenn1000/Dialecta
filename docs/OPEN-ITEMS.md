...
| ~~2.3~~ | ~~Three trigger functions still hold `anon` EXECUTE~~ **Closed 2026-09-21.** `set_updated_at`, `quotes_set_updated_at`, `profiles_subscription_tier_touch` closed to all of public, anon and authenticated (migration `20260922033108`), matching `check_handle_not_reserved`'s own shape; measured after, all three false. The future-function default is closed the same night (`20260922033110`): the global form for PUBLIC's built-in default, the per-schema form for anon's, per PostgreSQL's own documentation on why the two need opposite forms | Done |
