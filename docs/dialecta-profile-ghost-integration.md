...
Ghost's Members API returns a `member` object with fields like `name`, `email`, `created\_at`, and `profile\_image`. The React profile component expects a richer shape. This helper bridges them.
...
&#x20;   joined: new Date(m.created\_at).toLocaleDateString('en-US',
