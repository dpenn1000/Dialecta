...
| Check the major version a framework's current documentation is written against before copying from it. Current does not mean applicable, and the failure is usually silent | high | Next.js documents `proxy.ts` throughout because of a 16.0.0 rename. On 15.5.x that file is never invoked. Nothing in a build, a type check or a review reports a file the framework simply ignores, so the sample that looked safest is the one that fails open. `2026-nextjs-middleware-cve.md` | 2026-09-20 |
| Read a dependency's installed version, never its manifest range | high | `apps/web/package.json` declares `^2.0.0` for `@supabase/supabase-js` and 2.116.0 is installed. A hundred minor versions of difference decided whether passkeys were available for P0-D2. `2026-supabase-ssr-server-auth-methods.md` | 2026-09-20 |
...
### For the Next.js rebuild, read before any auth is written

| Position | Confidence | Why | Evidence |
| --- | --- | --- | --- |
| On `apps/web` the file is `middleware.ts` exporting `middleware`. Rename every official sample before using it | high | Next.js renamed this file to `proxy.ts` in 16.0.0 and now documents it that way everywhere, including the Authentication guide and the CSP nonce recipe. `apps/web` is on 15.5.25 and the convention came with a major version rather than a date, so no 15.5 release has it. A `proxy.ts` here is never invoked, which means an authorization check inside it does not fail, it never runs | `2026-nextjs-middleware-cve.md`, `2026-nextjs-csp-nonce-strict-dynamic.md` |
| Middleware refreshes the Supabase session and authorizes nothing | high | Vercel's own postmortem says middleware should not be the sole method of protecting routes, and the current Next.js Authentication guide says checks belong as close as possible to the data source. Both say it independently of CVE-2025-29927, which 15.5.25 is already patched against. Authorization belongs beside the query | `2026-nextjs-middleware-cve.md`, `2026-supabase-ssr-server-auth-methods.md` |
| Every Server Action carries its own authorization check as its first statement | high | A Server Action is a public HTTP endpoint callable by anyone holding its id. Importing it into a component that only renders for admins protects nothing. The rebuild routes every trust carrying write through these, so they are the perimeter rather than an implementation detail | `2026-nextjs-server-actions-authorization.md` |
| Column privileges, not policy text, are what stop a contributor writing their own tier | high | RLS is row level and cannot express a column limit, which is the whole of reviewer blocker B2. The two migrations the rebuild starts from carry 30 policies and zero `GRANT` or `REVOKE`. Withholding the grant makes the write impossible rather than forbidden | `2026-supabase-column-level-security.md`, `2026-live-grant-and-policy-surface.md` |
| Ship the static security header set now and defer the CSP nonce until something needs it | medium | Nothing in `apps/web` renders an inline script today, so `unsafe-inline` is not buying anything that is being used. When the Tiptap editor changes that, the policy needs `strict-dynamic` or Next's chunked loading breaks under it, and every page carrying a nonce becomes dynamically rendered, which is a real cost on a mostly static publication | `2026-nextjs-csp-nonce-strict-dynamic.md`, `2026-nextjs-security-headers-csp.md` |
