...
| D-13 | Topology segments cannot carry tier identity by colour alone. Stance and Breach are one colour under normal vision and under all three CVD types | High on the problem, open on the fix | `2026-dialecta-tier-palette-audit`, `2023-w3c-wcag22-colour-criteria` (SC 1.4.1). Two candidate fixes: icons in the segments, or `studio/001`, which moves identity off the bar entirely | 2026-09-20 |
...
| D-27 | Brass is a frame on paper and type only on dark. No brass stop clears 4.5:1 as text on cream or on the body stone gradient, so brass type belongs on `--dark-card` and on paper brass is a rule, a ring and a dot under the 3:1 floor of SC 1.4.11. `page-about.hbs` fails this today at `--card-brass` 1.85 on cream and `--hero-brass` 2.71 on the darkest stone | High | Own computation, 2026-09-20; reproducible from the token values in `_theme/assets/css/style.css` | 2026-09-20 |
| D-28 | Four of the ten rows in the subscription capability matrix are not tier capabilities at all, because they read Yes for both tiers. They describe the free tier and belong outside the comparison. Keeping them inside is why "eight of ten do not exist" reads worse than the structure warrants | High | `docs/SUBSCRIPTION-MODEL.md`, `_recovered/api/_subscription-tier.js`; `positions/2026-09-20-the-membership-page.md` | 2026-09-20 |
| D-29 | The membership mark composes from the record's three columns rather than being three badges: a seat ring for every member, brass plus a heavier stroke for Charter, a filled dot for Underwriter access, a terra seal for Honored (migration 033's word). Weight is redundant with hue because greyscale collapsed the two contrast-matched rings into one. Departs from 032's "faint" ring, which at 3.18:1 already sits on the 1.4.11 floor | High | Own render and computation, 2026-09-20; `1996-healey-effective-colours`, `2023-w3c-wcag22-colour-criteria`; migrations 032 and 033 | 2026-09-20 |
| D-30 | `is_gifted` carries two meanings. 033 defines it as a peer's gift and names the badge Honored; 034 also sets it for house comps (Charter Writers, Founding Voices). An Honored mark on a comped member claims a peer paid when nobody did, `gifted_by_member_id IS NULL` separates the two, and 034's expiry cron leaves `is_gifted` set after a lapse | High | Migrations 033 and 034 in `_recovered/supabase/migrations/` | 2026-09-20 |
| D-31 | The live candidate gate is 2 against 3. `max_candidates: null` gives Underwriters the skill default, target 3, where the brief and the doc's reading implied unlimited. The paid benefit that works today is thinner than stated, which strengthens the case for pricing on patronage | High | `_recovered/api/_subscription-tier.js`, FREE and UNDERWRITER blocks | 2026-09-20 |
| D-32 | Zero Charter Underwriters exist today. The one `pro` profile was toggled by an admin, and 031 reserves `set_by = 'system'` for the payment webhook, so any charter count must filter on it or it reads 99 | High | Migration 031, `subscription_tier_set_by` comment; `docs/SUBSCRIPTION-MODEL.md` live counts | 2026-09-20 |
