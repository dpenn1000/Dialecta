...
| **A hard constraint recorded 2026-05-29 says Dialecta must not be migrated or touched in any way**, naming its DNS and its GoDaddy Websites and Marketing, M365 Email Essentials and Conversations Deluxe subscriptions. Binding until Dan reopens it | High | `research/2026-personal-web-properties-memory.md` | 2026-09-20 |
| ~~Confirm the GoDaddy Domain Alert Pro or Auctions membership.~~ **Withdrawn 2026-09-20.** The $242 GoDaddy line is named in the constraint above. This advisor does not argue with a decision already recorded | High | `research/2026-personal-web-properties-memory.md` | 2026-09-20 |
| ~~Cancel Resend. $240 a year, two emails delivered, each to one recipient, while Magic Pages already bundles 10,000 a month.~~ **Withdrawn 2026-09-20.** Resend is the cheapest custom SMTP path for Supabase Auth, which the platform needs past the fourteenth member; Magic Pages' bundle is Ghost's own newsletter feature and cannot substitute for it. Hold the line until SMTP is wired | High | `research/2026-subscription-command-center.md`, `exchange/open/2026-09-19-002-blindspot-supabase-smtp-blocks-magic-link.md` | 2026-09-20 |
...
| **Dialecta's Supabase bill is probably not in the ledger at all, so the floor is higher than $78 rather than lower.** Supabase bills per organisation. The $32 line is annotated Trinity tools, this session's token is scoped to Trinity Solar, and Dialecta sits in Pennington Media Group, a separate organisation with its own bill | Medium-high | `research/2026-personal-web-properties-memory.md` | 2026-09-20 |
| ~~If Pennington Media Group is on Free tier the database is paused.~~ **Ruled out 2026-09-20.** The deployed profile API answered in 1.3 seconds with a live row. The database is awake, so the organisation is on a paid plan and its bill is missing from the ledger. The floor is above $78 | High | `exchange/open/2026-09-20-005` | 2026-09-20 |
| **`GET /api/profile/:id` creates a `profiles` row for any string, unauthenticated.** The P0-D2 abuse argument now has a source and it is worse than assumed: an attacker never needs to sign up. A rate limit on accounts is necessary and not sufficient | High | `exchange/open/2026-09-20-005` | 2026-09-20 |
| Read `C:\Users\dan\claude-memory` at the start of a sprint. Decisions that bind Dialecta live outside the Dialecta repo, and sprint 1 did not know that | High | `research/2026-personal-web-properties-memory.md` | 2026-09-20 |
...
| **A full paid-tier design, Underwriter, was recovered from a deployed build, not designed by this council.** Two-tier capability matrix, five migrations applied live (`subscription_tier`, Charter badge, peer gifting, lifetime grants). Unwired: `getTierCapabilities` has no caller, so every member gets free behaviour today regardless of the column | High | `exchange/open/2026-09-20-convener-01-handoff-underwriter-tier-recovered.md` | 2026-09-20 |
| Underwriter's free tier gates editor depth only (opinion-map candidates, polish runs, archive past three), never reading or commenting. That is this position's own "gate nothing, the contributor is never the cost problem" argument, reached independently before this position existed | High | same | 2026-09-20 |
| Peer gifting comps the recipient through the Ghost Admin API. Dialecta is leaving Ghost, so that webhook call is dead; the Stripe one-time-payment leg and the `is_gifted`/`gift_expires_at` columns survive and need rebuilding against Supabase Auth and Stripe directly. Nobody has sized that build | High | same, Dan's 2026-09-20 amendment | 2026-09-20 |
| No price exists anywhere in the recovered design (`upgrade_url` is null in both tiers). Pricing is Dan's call for M2, not set by this position | High | same | 2026-09-20 |
...
| ~~A prior subscription model plan may exist that this advisor has not read.~~ **Closed 2026-09-20, corrected same day.** The `subscription-command-center` find still stands: expense tracker, not a revenue model, and Ghost Tiers was never configured (Dan confirmed directly; Dialecta is leaving Ghost entirely). **"No membership model has ever been designed" does not stand.** A full tier design, Underwriter, was recovered from a deployed build the same day, five migrations already applied live. See `positions/monetization.md` | `research/2026-subscription-command-center.md`, `research/2026-search-for-the-subscription-plan.md`, `exchange/open/2026-09-20-convener-01-handoff-underwriter-tier-recovered.md` |
