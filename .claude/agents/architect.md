...
| --- | --- | --- |
| **Supabase** | `supabase-dialecta-ro`, a connector that is read-only on Supabase's side (`read_only=true`), for any SQL you need to run. Plus the schema metadata tools from the main connector as a fallback, including `generate_typescript_types`, which can regenerate the stale `supabase/types.ts` without writing anything | `execute_sql` on the main connector. Measured 2026-09-21, it runs as `supabase_read_only_user` with write privilege on none of the 31 public tables, so it behaves read-only; its own description says a destructive statement "may require the user to confirm", which makes that its behaviour rather than its guarantee, and `supabase-dialecta-ro` is the guarantee. (This row first said it "runs any statement at all". That was this seat's claim, unmeasured, and the measurement refuted it.) Also `apply_migration`, `deploy_edge_function` and anything that creates, merges or resets a branch |
