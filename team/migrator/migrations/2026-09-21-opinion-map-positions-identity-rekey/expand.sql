-- Proposed path: supabase/migrations/<TIMESTAMP>_opinion_map_positions_identity_expand.sql
-- Convener: replace <TIMESTAMP> with the real YYYYMMDDHHMMSS at apply time (UTC, after the
-- last-applied migration). Not applied by this seat. Reads only were run to write this file;
-- no DDL has touched project mguulnibvzusfvyuowwh.
--
