...
| `migration-history.sql` | Every live migration has a tracked file under the same version, and the file says what ran | 2 of 31 matched by version; 3 renamed; 3 with no file anywhere; the unapplied baseline read as pending by the CLI | Every version matches, and `md5_sql_only` matches the file |
