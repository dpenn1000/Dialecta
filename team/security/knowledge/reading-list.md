...
| todo | The pinned `@supabase/supabase-js` version in `apps/web` | Passkeys need a recent minimum and `getClaims()` needs the newer signing key support. Two filed notes assume availability that the app's own lockfile may not have |
